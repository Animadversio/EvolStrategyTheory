#!/usr/bin/env python3
"""Exp D: Multi-step ES on quadratic landscape — theory validation and GD comparison.

Theory (from theory.tex §Multi-step analysis):
  θ_{t+1} = (I - γ Q) θ_t + η_t,  γ = ασ/σ_R(θ_t)
...
def theory_noise_floor(lam, sigma, alpha, N, xi, d):
    """
    Stationary noise floor for each eigenmode.

    Near θ*=0: v = Qθ → 0, so σ_R² ≈ ½σ⁴ Tr[Q²] + ξ²  (curvature-dominated).
    Each mode c_i = u_i^T θ satisfies a discrete OU process:
        c_i(t+1) = (1 - γ* λ_i) c_i(t) + noise_i(t)

    The noise variance per mode has two contributions (from Cov[η_t]):
      - Isotropic: α²/N  (from σ_R² I_d term)
      - Anisotropic curvature: 2σ⁴ α²/(N σ_R²) * λ_i²  (from 2σ⁴Q² term)

    Stationary variance (exact for linear OU):
        Var[c_i(∞)] = noise_var_i / (1 - (1-γ*λ_i)²)

    Total noise floor: E[‖θ_∞‖²] = Σ_i Var[c_i(∞)]
    """
    lam_np = lam.cpu().numpy()
    ...
    # Per-mode noise variance: isotropic + anisotropic Q² term
    noise_iso   = alpha**2 / N                                   # scalar
    noise_Q2_i  = 2.0 * sigma**4 * alpha**2 / (N * sigma_R0**2) * lam_np**2  # (d,)
    noise_var_i = noise_iso + noise_Q2_i                         # (d,)

    # Stationary variance per mode (OU: σ²_∞ = noise_var / (1-(1-γλ)²))
    denom     = np.maximum(1.0 - (1.0 - gamma0 * lam_np)**2, 1e-10)
    stat_vars = noise_var_i / denom

    return stat_vars.sum(), gamma0, sigma_R0, stat_vars
...
def exp_D3(traj_coords, sigma_R_traj, theta0, U, lam, args, gamma0, figs, data, tag):
    print("\n[D3] Noise floor — stationary distribution", flush=True)
    T   = args.T
    d   = args.d
    lam_np = lam.cpu().numpy()
    t_ax   = np.arange(T + 1)

    # Empirical variance per mode over trials at each time step
    emp_var = traj_coords.var(axis=0)       # (T+1, d)  variance across trials
    emp_var_total = emp_var.sum(axis=-1)    # (T+1,)    total Var[‖θ‖²] proxy

    # Empirical mean ‖θ‖² (includes both mean² and var)
    emp_norm2 = (traj_coords ** 2).sum(axis=-1).mean(axis=0)  # (T+1,)

    # Theory noise floor
    noise_floor_total, gamma_star, sigma_R_star, stat_var_theory = theory_noise_floor(
        lam, args.sigma, args.sigma / 2.0, args.N, args.xi, d)

    print(f"  γ* (near θ*): {gamma_star:.5f},  σ_R*: {sigma_R_star:.4f}", flush=True)
    print(f"  Theory noise floor ‖θ_∞‖²: {noise_floor_total:.2f}", flush=True)
    print(f"  Empirical ‖θ_T‖² (last 20% steps): {emp_norm2[int(0.8*T):].mean():.2f}", flush=True)

    fig, axes = plt.subplots(1, 3, figsize=(15, 4))

    # D3a: ‖θ‖² over time
    ax = axes[0]
    ax.plot(t_ax, emp_norm2, lw=2, label="E[‖θ_t‖²]")
    ax.axhline(noise_floor_total, color="r", ls="--", lw=1.5, label=f"Theory floor={noise_floor_total:.1f}")
    ax.set_xlabel("Step t"); ax.set_ylabel("E[‖θ_t‖²]")
    ax.set_title("D3a: Convergence to noise floor")
    ax.legend(); ax.grid(alpha=0.3)

    # D3b: Variance across trials per mode (last 20% steps = stationary)
    stationary_var = emp_var[int(0.8 * T):].mean(axis=0)  # (d,)
    ax = axes[1]
    mode_idx = np.arange(min(args.k + 5, d))
    ax.semilogy(mode_idx, stationary_var[mode_idx], "o-", ms=5, lw=1.5, label="Empirical Var[c_i(∞)]")
    ax.semilogy(mode_idx, stat_var_theory[mode_idx], "x--", ms=5, lw=1.5, label="Theory")
    ax.set_xlabel("Eigenmode index i"); ax.set_ylabel("Stationary variance")
    ax.set_title("D3b: Per-mode stationary variance")
    ax.legend(); ax.grid(alpha=0.3)

    # D3c: Total variance over time
    ax = axes[2]
    ax.plot(t_ax, emp_var_total, lw=2, label="Var[‖θ_t‖] (sum over modes)")
    ax.axhline(stat_var_theory.sum(), color="r", ls="--", lw=1.5,
               label=f"Theory Σ={stat_var_theory.sum():.1f}")
    ax.set_xlabel("Step t"); ax.set_ylabel("Total variance")
    ax.set_title("D3c: Total variance over time")
    ax.legend(); ax.grid(alpha=0.3)

    fig.suptitle(f"D3 Noise Floor  d={args.d} N={args.N} σ={args.sigma} ξ={args.xi}")
    savefig(fig, figs / "expD3_noise_floor.png", tag)

    res = {
        "emp_norm2": emp_norm2, "emp_var": emp_var,
        "stationary_var_emp": stationary_var, "stat_var_theory": stat_var_theory,
        "noise_floor_total": noise_floor_total,
        "gamma_star": gamma_star, "sigma_R_star": sigma_R_star,
        "lam": lam_np, "t_ax": t_ax, "args": vars(args),
    }
    pkl = data / f"expD3_noise_floor{('_'+tag) if tag else ''}.pkl"
    with open(pkl, "wb") as f: pickle.dump(res, f)
    print(f"  Data: {pkl}", flush=True)
    return res
